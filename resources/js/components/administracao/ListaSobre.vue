...
<div>    
    ...
    <div class="col-md-12" v-if="sms">
       ...
       <div class="col-md-12" v-for="erro in erros" :key="erro.id">
                           
                            <div class="alert alert-warning alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>{{erro}}</div>
    </div>

  <div class="form-inline" >
  <a v-if="criar && !modal" v-bind:href="criar">Criar</a>
      <modal_link v-if="criar && modal" tipo="button" nome="adicionar" titulo="Sobre" css="btn btn-success"></modal_link>    
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                       ...
                       <tr v-for="(item,index) in lista" :key="item.id">
                        ...
                        <td>{{item.id}}</td> 
                         <td><img :src="'http://localhost:82/nossafrica/storage/app/public/'+item.sobre_img" alt="" class="img-responsive img-rounded img-thumbnail" width="120px"></td> 
                         <td v-html="item.sobre_descricao"></td>
                          <td>{{item.sobre_titulo}}</td>
                         <td>{{item.sobre_data}}</td>
                          
                      
                <td v-if="item.user_status==1">  
                <a v-if="status && !modal" v-bind:href="status" class="btn btn-info btn-xs"> Activar </a>
               <modal_link v-if="status && modal" v-bind:url="status" v-bind:item="item" tipo="button" nome="status" css="btn btn-xs btn-success" titulo="Activar" clas="fa fa-eye"></modal_link>
                </td> 
                   <td v-if="item.user_status==0">  
                <a v-if="status && !modal" v-bind:href="status" class="btn btn-xs btn-warning"> Inativo </a>
               <modal_link v-if="status && modal" v-bind:url="status" v-bind:item="item" tipo="button" nome="status" css="btn btn-xs btn-warning" titulo="Inativo" clas="fa fa-eye"></modal_link>
                      </td> 
...
<script>
    ...
    export default {
     props:['titulos','itens','criar','editar','deletar','token','status','modal','ordem','ordemcol','sms','erros'],
       data: function(){
